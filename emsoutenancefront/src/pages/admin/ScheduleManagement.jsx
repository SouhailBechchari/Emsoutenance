...
export default function ScheduleManagement() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Récupérer seulement les rapports validés
  ...
  // Filtrer les étudiants pour n'afficher que ceux avec rapport validé
  const studentsWithValidReports = students.filter(student => {
  ...
  const handleStudentSelect = (studentId) => {
    ...
    // Trouver le rapport validé de l'étudiant
    const studentReport = reports.find(r => 
      parseInt(r.student_id) === parseInt(studentId) && r.status === 'validated'
    );
                    ...
                    <tr key={defense.id} className="hover:bg-gray-700 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 rounded text-xs ${
                          defense.status === 'scheduled' ? 'bg-blue-900 text-blue-300' :
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  {studentsWithValidReports.length === 0 ? (
                    <option value="" disabled>Aucun étudiant avec rapport validé</option>
                  ) : (
                    studentsWithValidReports.map(student => (
